...
inline void addN2Queue(queue<SpinStruct*> &Q, SpinStruct &curSpin, double pBond, 
		TRandom3 *inRand, bool spinVal);
void initSpins(vector<vector<vector<SpinStruct>>> &spinsIn, const unsigned int length, 
		bool isRand); 
long double exactSus(double jBeta, unsigned int LATLEN, TH1D &hist1, TGraph &graph1, 
		double g1norm);
...
inline void elapsedTime(string nameString, high_resolution_clock::time_point t1, 
...
int main(int argc, const char *argv[]) {
	...
	unsigned int NSWEEP = (unsigned int) atoi(argv[3]);
	unsigned short version = 0;
	if(argc >= 5) version = atoi(argv[4]);
	unsigned int updates = 10000;
	if(argc>=6) updates = atoi(argv[5]);
	bool fitOut = false;
  ...
  unsigned int swpCnd[3] = {10*Volume,0,updates}; //Based on distributions
  unsigned int printLim = 0; // How many sweeps are printed out
	...
	//TH1::SetDefaultSumw2();
	TH1D susDist("susDist","Susceptibility Distribution",1000,0,Volume+1);
	TH1D SijNdist("SijNdist","Density of States for 2x2x2 Ising Model",50,-25,25);
	TH1I withinSweep("withinSweep","Distribution of Cluster Sizes",1000,0,Volume+1);
	TH1I withinSweepSel("withinSweepSel","Distribution of Cluster Sizes",Volume+1,0,Volume+1);
	TH1I statesMC("statesMC","MC Distribution of States",pow(2,8)+1,0,pow(2,8)+1);
	TH1I checkDist("checkDist","Dist of Queue Size",7,0,7);
	TGraph statesExact(pow(2,Volume));
	TTree *convergenceTree = new TTree("convergenceTree","convergenceTree");
  //Initialize spin lattice randomly, the last index:if 0 spin,if 1 in cluster flag
	...
	unsigned int clSize; // cluster size within an update
	unsigned long int sumClSize; // Sum of all cluster sizes obtained within the sweep
	unsigned int clSizeMsCnt; // Number of cluster size obtained in sweep
	unsigned long int totalClCnt; 
	...
	bool measFlag;
	unsigned int NSKIP = 1+Volume/swpCnd[2];
	unsigned int NMEAS = NSWEEP;
	unsigned int swpctr;
	NSWEEP = NSWEEP+NSKIP;
	if(!fitOut)  cout << "Skipping first: " << NSKIP << " measurements. Total sweeps became: "
			<<	NSWEEP << endl;
	TGraph sweepEv(NSWEEP);
	double spinSus[NSWEEP];
	double spinSusMean=0,spinSusStd=0,spinSusRMS=0,spinSusError;
	int count1 = 0;
	bool spinVal;
	// Tree branches
	convergenceTree->Branch("clSize", &clSize, "clSize/i");
	convergenceTree->Branch("sweep", &swpctr, "sweep/i");
	//Timer
	high_resolution_clock::time_point t1;
	high_resolution_clock::time_point t2;
	//Main Loop (Sweeps)
	for(swpctr=0;swpctr<NSWEEP;swpctr++) {
		...
		while(true) { // While loop for updates within a sweep
			...
			//t2 = high_resolution_clock::now();
			//elapsedTime("Repeating all for neighbours: ",t1,t2);
			//cout << "Cluster Size is: "<< clSize << endl;
			...
			//t1 = high_resolution_clock::now();
			//if(!spinVal) count1+=clSize;
			//else count1-=clSize;
			sumClSize+=clSize;
			clSizeMsCnt++;
			//totalClCnt = clSizeMsCnt+swpCnd[2]*swpctr;
			//convergenceTree->Fill();
			//t2 = high_resolution_clock::now();
			//elapsedTime("Modifying Observables: ",t1,t2);
		...
		//spinSus[swpctr] = pow(((double)Volume - (double)(count1*2)),2)/(double)Volume;
		spinSus[swpctr] = (double)sumClSize/(double)clSizeMsCnt;
		sweepEv.SetPoint(swpctr,swpctr,spinSus[swpctr]);
		cout << swpctr << " " << spinSus[swpctr] << endl; // For Debug
	}
	// Calculate Average/Std. Dev.
	for (unsigned int swpctr = NSKIP; swpctr < NSWEEP; swpctr++) {
		susDist.Fill(spinSus[swpctr]);
		spinSusMean+=spinSus[swpctr];
		spinSusRMS+=pow(spinSus[swpctr],2);
	}
	spinSusMean/=(double)NMEAS;
	spinSusRMS/=(double)NMEAS;
	spinSusStd = sqrt(spinSusRMS - pow(spinSusMean,2));
	spinSusError = spinSusStd/sqrt((double)NMEAS);
  if(!fitOut) {
  	if(LATLEN <= MAXEXACT) { //Print Exact Result
  		long double spinSusExact = exactSus(jBeta,LATLEN,SijNdist,statesExact,
  				statesMC.Integral()); 
  		cout << "Exact Result is: " << spinSusExact << endl;
		}
...
inline void addN2Queue(queue<SpinStruct*> &Q, SpinStruct &curSpin, double pBond, 
	...
	for(unsigned int nbrcnt=0;nbrcnt<NNBRS;nbrcnt++) {
		if(curSpin.nbrs[nbrcnt]->value == spinVal && inRand->Rndm() < pBond) {
			curSpin.nbrs[nbrcnt]->value = !spinVal;
...
void initSpins(vector<vector<vector<SpinStruct>>> &spinsIn,
		const unsigned int length, bool isRand) { 
	TRandom3 initializer;
...
inline void elapsedTime(string nameString, high_resolution_clock::time_point t1, 
	...
	std::cout << nameString << time_span.count() << " seconds." << endl;
}
